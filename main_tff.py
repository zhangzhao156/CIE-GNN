...
# import models
from models2.CausalGCN import CLUB, CausalGCN, CausalGAT, CausalGIN
from TFFgraph import TFFgraph
...
def parse_args():
    ...
    parser.add_argument('--noise_std', type=float, default=0.07, help='the noise added in test data')
...
def eval_acc_causal3(model, loader, device, args):
    ...
    acc_c = correct_c / len(loader.dataset)
    acc_o = correct_o / len(loader.dataset)
    np.save('edgatt_CGAT', edgatt.cpu().numpy())
    np.save('nodeatt_CGAT', nodeatt.cpu().numpy())
    np.save('labels_CGAT', (data.y.view(-1)).cpu().numpy())
...
if __name__ == '__main__':
    ...
    datasets = {}
    datasets['train'], datasets['val'] = TFFgraph(args.sample_length, args.overlap, args.noise_std).data_preprare()
    ...
    random_guess = 1.0 / TFFgraph.num_classes
    ...
    if args.model_name == "CausalGCN":
        model = CausalGCN(num_features=feature, num_classes=TFFgraph.num_classes, args=args,gfn=False,edge_norm=True).to(device)
    elif args.model_name == "CausalGIN":
        model = CausalGIN(num_features=feature, num_classes=TFFgraph.num_classes, args=args, gfn=False, edge_norm=True).to(
        device)
    elif args.model_name == "CausalGAT":
        model = CausalGAT(num_features=feature, num_classes=TFFgraph.num_classes, args=args).to(
        device)
    else:
        print('No model')
